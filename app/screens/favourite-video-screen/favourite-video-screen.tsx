...
export const FavouriteVideoScreen = observer(function FavouriteVideoScreen({ route }) {
  ...
  useEffect(() => {
    isSetVideoId(route.params.data.item.video_id._id)
    ...
    viewComment(route.params.data.item.video_id._id)
  ...
  const reportContent = async (category: any) => {
    let video = {
      user_id: user.user_id,
      video_id: route.params.data.item.video_id._id,
      flag_type: "video",
      tokken: user.token,
      category: category,
    }
  ...
  return (
    <Screen style={ROOT} preset="scroll">
            ...
            <>
              <TouchableHighlight>
                <Video
                  ...
                  source={{ uri: route.params.data.item.video_id.video }}
                  ref={videoRef}
                  // poster={route.params.data.thumbnail?.path}
                  // posterResizeMode={'stretch'}
                  onEnd={() => onEnd(route.params.data.item.video_id._id)}
      ...
      <View style={{ padding: 10 }}>
        <Text text={route.params.data.item.video_id.title} style={{ color: "black", fontWeight: "bold" }} />
      </View>
          ...
          <Text
            text={
              views && views.length > 0
                ? views.length + " " + "Views"
                : route.params.data.item.video_id.views?.length + " " + "Views"
            }
            style={{ color: "#7F7F7F" }}
          />
        </View>
        <TouchableOpacity
          onPress={() => likeVideo(route.params.data.item.video_id._id)}
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <View style={{ backgroundColor: "#C5C5C5", height: 70, width: 70, borderRadius: 40 }}>
              <Image
                source={{ uri: route.params.data.item.user_id.profile_picture }}
            ...
            <Text
              text={
                route.params.data.item.user_id.first_name + " " + route.params.data.item.user_id.last_name
              }
